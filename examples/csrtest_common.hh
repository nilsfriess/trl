...
{
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0] << " <matrix.mtx> [nev] [ncv] [tol]\n";
    ...
    std::cerr << "  tol: Relative tolerance (default 1e-8)\n";
    return 1;
  }
  ...
  const double tol = (argc > 4) ? std::stod(argv[4]) : 1e-8;
  ...
  try {
    ...
    trl::EigensolverParams params{.nev = nev, .ncv = ncv, .max_restarts = 1000, .tolerance = tol};
    ...
    for (std::size_t i = 0; i < num_runs; ++i) {
      ...
      eigs.compute(Spectra::SortRule::LargestMagn, 1000, tol);
    ...
    for (std::size_t i = 0; i < num_runs; ++i) {
      trl::BlockLanczos<EVP, trl::ModifiedGS> lanczos(evp, params);
...
template <unsigned int BlockSize, class CreateEvp, class PrintBackendInfo, class InitializeBlock, class Synchronize>
int run_generalised_csrtest(int argc, char* argv[], CreateEvp&& create_evp, PrintBackendInfo&& print_backend_info, InitializeBlock&& initialize_block,
                            Synchronize&& synchronize)
{
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0] << " <matrixA.mtx> <matrixB.mtx> [nev] [ncv] [tol]\n";
    std::cerr << "  matrixA.mtx: Path to a Matrix Market file (the left-hand side matrix)\n";
    std::cerr << "  matrixB.mtx: Path to a Matrix Market file (the right-hand side matrix)\n";
    std::cerr << "  nev: Number of eigenvalues to compute (default: 10)\n";
    std::cerr << "  ncv: Number of Lanczos vectors (default: 4 * nev)\n";
    std::cerr << "  tol: Relative tolerance (default 1e-8)\n";
    return 1;
  ...
  const std::string matrix_A = argv[1];
  const std::string matrix_B = argv[2];
  const unsigned int nev = (argc > 3) ? std::atoi(argv[3]) : 16;
  const unsigned int ncv = (argc > 4) ? std::atoi(argv[4]) : 4 * nev;
  const double tol = (argc > 5) ? std::stod(argv[5]) : 1e-8;

  std::cout << "========================================\n";
  std::cout << "    CSR Sparse Matrix Eigensolver      \n";
  std::cout << "========================================\n";
  std::cout << "Matrix files: A = " << matrix_A << ", B = " << matrix_B << "\n";
  std::cout << "nev = " << nev << ", ncv = " << ncv << ", blocksize = " << BlockSize << "\n";

  try {
    auto evp = create_evp(matrix_A, matrix_B);
    print_backend_info();
    ...
    Eigen::loadMarket(A, matrix_A);
    Eigen::loadMarket(B, matrix_B);
