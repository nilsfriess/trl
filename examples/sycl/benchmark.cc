...
std::pair<T, std::chrono::duration<double>> run(sycl::queue &queue, std::size_t n, const RunParams &run_params)
{
  ...
  std::vector<T> correct(blocksize * blocksize, 0);
  for (std::size_t i = 0; i < blocksize; ++i)
    for (std::size_t j = 0; j < blocksize; ++j)
      for (std::size_t k = 0; k < n; ++k)
        correct[i * blocksize + j] += u.data[k * blocksize + i] * v.data[k * blocksize + j];

  // Verify
  u.template dot<worker_mult>(v, r);
  queue.wait();
  const auto tol = std::is_same_v<T, double> ? 1e-10 : 1e-4;
  for (std::size_t i = 0; i < blocksize * blocksize; ++i)
    if (auto err = std::abs(correct[i] - r.data[i]); err > tol) {
      std::cout << "ERROR: Result is wrong: " << err << "\n";
      break;
    }

  for (int i = 0; i < run_params.n_warmup; ++i)
    u.template dot<worker_mult>(v, r);
  queue.wait();

  auto start = std::chrono::steady_clock::now();
  for (int i = 0; i < run_params.n_benchmark; ++i)
    u.template dot<worker_mult>(v, r);
  queue.wait();
...
void measure(sycl::queue &queue, std::size_t n, const RunParams &params)
{
  ...
  std::cout << "  worker_mult=" << worker_mult << ": " << time.count() * 1000. << "ms, " << flops(n, bs) / time.count() / 1e9 << " GFLOP/s\n";
}
...
void measure_all_worker_mult(sycl::queue &queue, std::size_t n, const RunParams &params)
{
  ...
  measure<Scalar, bs, 1024>(queue, n, params);
}
...
int main()
{
  ...
  std::size_t n = 1 << 20;
  RunParams params{.n_warmup = 10, .n_benchmark = 20};
