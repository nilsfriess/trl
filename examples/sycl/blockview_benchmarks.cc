...
void print_usage(const char* program)
{
  std::cout << "Usage: " << program
            << " <benchmark_number> [stream_bandwidth_MBps] [peak_tflops] [runs] [cols]\n"
            << "\n"
            << "Benchmarks:\n"
...
{
  return BenchmarkMetrics{2.0 * static_cast<double>(rows) * cols * cols,
                          dot_arithmetic_intensity_flops_per_byte<Scalar, cols>()};
}
...
Scalar max_abs_error(const Scalar* actual, const std::array<Scalar, size>& expected)
{
  ...
  for (std::size_t index = 0; index < size; ++index)
    max_err = std::max(max_err, std::abs(expected[index] - actual[index]));
  return max_err;
}
...
void report_benchmark(double avg_time, const BenchmarkMetrics& metrics,
                      const RooflineConfig& roofline)
{
  ...
  const double roofline_flops_per_s =
      std::min(roofline.peak_flops_per_s,
               metrics.arithmetic_intensity_flops_per_byte * roofline.stream_bandwidth_bytes_per_s);
...
void run_blockview_benchmark(::sycl::queue& q, std::size_t rows, int runs, Operation&& operation,
                             ReferenceFactory&& make_reference, const BenchmarkMetrics& metrics,
                             const RooflineConfig& roofline)
{
...
void run_multivector_output_benchmark(::sycl::queue& q, std::size_t rows, int runs,
                                      Operation&& operation, ErrorComputer&& compute_error,
...
template <class Scalar, unsigned int cols>
void run_dot_benchmark(::sycl::queue& q, std::size_t rows, int runs,
                       const RooflineConfig& roofline)
{
...
template <class Scalar, unsigned int cols>
void run_gemm_benchmark(::sycl::queue& q, std::size_t rows, int runs,
                        const RooflineConfig& roofline)
{
...
int main(int argc, char* argv[])
{
  ...
  const std::optional<unsigned int> cols_filter = argc >= 6
                                                      ? std::optional<unsigned int>(std::stoul(argv[5]))
                                                      : std::nullopt;
  ...
  auto run_for_cols = [&]<unsigned int cols>() {
    ...
    switch (benchmark_number) {
      case 1:
        run_dot_benchmark<Scalar, cols>(q, rows, runs, roofline);
        break;
      case 2:
        run_gemm_benchmark<Scalar, cols>(q, rows, runs, roofline);
        break;
      default:
        std::cout << "Unknown benchmark number '" << benchmark_number << "'\n";
        break;
    }
  };
