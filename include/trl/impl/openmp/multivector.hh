...
public:
      ...
      , blocks_(cols / blocksize)
  {
    ...
    data = static_cast<Scalar*>(std::aligned_alloc(64, aligned_bytes));
  ...
  BlockMultivector(BlockMultivector&& other)
  {
    ...
    other.data = nullptr; // Prevent deallocation of moved-from object
  }
  ...
  ~BlockMultivector() { std::free(data); }

  BlockView block_view(std::size_t i) const { return BlockView(data + i * rows * block_size, rows); }
