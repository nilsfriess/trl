...
namespace trl::sycl {
          ...
          cgh.parallel_for(::sycl::range<1>{n}, [=](::sycl::item<1> it) {
            ...
            for (unsigned int i = 0; i < m; ++i)
              for (unsigned int j = 0; j < m; ++j)
                C_local[i][j] += a[to_linear_index(m, tid, i)] * b[to_linear_index(m, tid, j)];

            for (unsigned int i = 0; i < m; ++i)
              for (unsigned int j = 0; j < m; ++j) {
                ::sycl::atomic_ref<T, ::sycl::memory_order::relaxed, ::sycl::memory_scope::device> atomic_c(
                  c[to_linear_index(m, i, j)]);
  ...
  private:
    ...
    static auto make_reductions(T *c, std::index_sequence<Is...>) {
      return std::make_tuple(::sycl::reduction(c + Is, T{0}, ::sycl::plus<T>{},
                                               ::sycl::property_list{
    ...
    static void combine_reductions(const T *a, const T *b, std::size_t m, std::size_t k, std::index_sequence<Is...>,
                                   Reducers &... reducers) {
